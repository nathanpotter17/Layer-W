...
//! A cross-platform game and application framework that works on browser WASM, WASI, and native platforms.
...
/// Common error type for the LAYER-W framework
#[derive(Debug)]
pub enum LayerWError {
    /// Error coming from the SDL library
...
impl fmt::Display for LayerWError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            #[cfg(not(target_arch = "wasm32"))]
            LayerWError::SdlError(msg) => write!(f, "SDL error: {}", msg),
            LayerWError::GenericError(msg) => write!(f, "Error: {}", msg),
        }
    }
}

impl Error for LayerWError {}

#[cfg(not(target_arch = "wasm32"))]
impl From<String> for LayerWError {
    fn from(err: String) -> Self {
        LayerWError::SdlError(err)
    }
}

/// Result type used throughout the LAYER-W framework
pub type Result<T> = std::result::Result<T, LayerWError>;

/// Main engine structure that ties all LAYER-W systems together
pub struct Engine {
...
impl Engine {
    ...
    pub fn new() -> Result<Self> {
        ...
        input_system.init().map_err(|e| LayerWError::GenericError(e))?;
    ...
    pub fn update(&mut self) -> Result<()> {
        ...
        self.input_system.update().map_err(|e| LayerWError::GenericError(e))?;
